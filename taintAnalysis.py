...
class taintAnalysis:
    ...
    def extract_methods(self, node, current_class, file_path):
        ...
        for sub_path, sub_node in node:
            ...
            self.extract_variables(sub_node, current_class, method_name, count)
            
    
    def extract_variables(self, sub_node, current_class, method_name, count):
        # 변수 선언 및 정의일 때
        if isinstance(sub_node, javalang.tree.VariableDeclarator): 
            if isinstance(sub_node.initializer, javalang.tree.MethodInvocation):
                ...
                if invoked_method in self.source_functions:  
                    self.tainted_variables.append((f"{current_class}.{method_name}", sub_node.name , count))
        #변수 할당일 때
            ...
            if isinstance(sub_node.value, javalang.tree.MethodInvocation):
                ...
                if invoked_method in self.source_functions: 
                    self.tainted_variables.append((f"{current_class}.{method_name}", sub_node.expressionl.member , count))


    def append_flow(self):
        for class_method, var, count in self.tainted_variables:
            self.flow.clear()
            self.track_variable_flow(class_method, var, count)
            
            # 새로운 키를 생성하고, 기존 키가 존재하면 새 키를 사용
            existing_key = (class_method, var)
            new_key = self.numbering(self.flows, existing_key)

            # 만약 이미 해당 키가 있다면 기존 값에 flow를 추가
            self.flows[new_key].extend(self.flow)


    def numbering(self, d, key_tuple):
        if key_tuple in d:
